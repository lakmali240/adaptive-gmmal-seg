...
IMAGE_VAL_DIRECTORY = "results/fully_supervised_validation_images"
IMAGE_TEST_DIRECTORY = "results/fully_supervised_test_images"
...
def main():
    ...
    model, optimizer, scheduler, start_epoch = load_pretrained_model(model, optimizer, scheduler)
    
    # Training loop
    train_model(
        model, train_loader, val_loader, test_loader, optimizer, 
        scheduler, loss_fn, model_path,  val_image_path, test_image_path, start_epoch
    )
...
def train_model(model, train_loader, val_loader, test_loader, optimizer, scheduler, loss_fn, model_path, val_image_path, test_image_path, start_epoch):
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        results_df.to_csv(os.path.join(model_path, 'train_results.csv'))
        ...
        if valid_loss < best_loss:
            ...
            save_fss_predictions_images(val_loader, model, folder=val_image_path, device=DEVICE)
        ...
        # After training is complete, run testing on the best model
        print("\n--- Testing Best Model After Training ---")
        # Load the best model for  testing
        best_model = UNET(in_channels=3, out_channels=1).to(DEVICE)
        best_checkpoint = torch.load(os.path.join(model_path, "fully_supervised_learning.pt"), map_location=DEVICE)
        best_model.load_state_dict(best_checkpoint['state_dict'])
        
        # Run test for each epoch where we have training data
        print(f"\n--- Running test evaluation for epoch {epoch} ---")
        test_loss, test_dice = test_fn(test_loader, best_model, loss_fn, DEVICE)
        
        # Store results
        # Create a unique key for each epoch 
        test_result_key = f"{epoch}"
        
        # Store results
        test_results[test_result_key] = {
            'iteration': epoch,
            'current_learning_rate': train_results[str(epoch)]['current_learning_rate'],
        ...
        save_fss_predictions_images(test_loader, best_model, folder=test_image_path, device=DEVICE)                

...
def load_and_review_data(train_transform, val_transforms, test_transforms, config):
    ...
    train_loader, val_loader, test_loader, updated_clusters_file, selected_clusters_file = data_loader_for_self_supervised_assisted_active_learning(
        ...
        top_n_samples=300  # Select top 300 samples
    )
    
    
    df = pd.read_csv(updated_clusters_file)
    df.to_csv(os.path.join("results/gmm_results/2025-04-22_03-38-29", "ranked_cluster_assignments_updated.csv"), index=False)
    print(f"Saved updated clusters to {os.path.join("results/gmm_results/2025-04-22_03-38-29", 'ranked_cluster_assignments_updated.csv')}")

  
    df = pd.read_csv(selected_clusters_file)
    df.to_csv(os.path.join("results/gmm_results/2025-04-22_03-38-29", "ranked_cluster_assignments_selected.csv"), index=False)
    print(f"Saved selected clusters to {os.path.join("results/gmm_results/2025-04-22_03-38-29", 'ranked_cluster_assignments_selected.csv')}")
